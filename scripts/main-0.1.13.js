import { MODULE_ID, MODULE_TITLE } from "./constants.js?v=0.1.13";
import { loadTemplatesCompat, reportError } from "./foundry-compat.js?v=0.1.13";
import { registerSettings } from "./settings.js?v=0.1.13";
import { TimelineController } from "./timeline-controller.js?v=0.1.13";
...
Hooks.once("init", () => {
  ...
  void import("./actor-config.js?v=0.1.13")
...
Hooks.once("ready", () => {
  ...
  if (module) {
    module.api = {
      ...
      openCountdownConfig: async () => {
        ...
        try {
          const { CountdownConfigApplication } = await import("./countdown-config.js?v=0.1.13");
  ...
  console.info(`${MODULE_TITLE} | Ready 0.1.13`);
});
